...
static int prepare_uuid_var(char *fd_ptr, char *env_var_name)
{
...
static void factory_data_env_config(void)
{
	...
	env_updated |= prepare_uuid_var(fd->ipr_id, "linuxmoduleid");
	env_updated |= prepare_uuid_var(fd->hqv_id, "linuxmodulehqvid");
	env_updated |= prepare_uuid_var(fd->unielec_id, "linuxmoduleunielecid");
...
int do_fd_write(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	 ...
	 */
	printf("ipr_id=%." __stringify(UUID_STR_LEN) "s\n", fd->ipr_id);
	printf("hqv_id=%." __stringify(UUID_STR_LEN) "s\n", fd->hqv_id);
	printf("unielec_id=%." __stringify(UUID_STR_LEN) "s\n", fd->unielec_id);
